    ...
    <div class="flex flex-wrap justify-center">
        <div class="w-full max-w-sm">
            <div class="flex flex-col break-words bg-white border border-2 shadow-md mt-20">
                ...
                @if (session('status'))
                <span class="bg-green-100 text-center text-green-700 p-4 w-full mt-5 text-sm " role="alert">
                    ...
                    <form class="py-10 px-5" method="POST" action="{{ route('password.email') }}" novalidate>
                        ...
                        <div class="flex flex-wrap mb-6">
                            <label for="email" class="block text-gray-700 text-sm mb-2">{{ __('E-Mail Address') }}</label>
                                ...
                                <input id="email" type="email" class="p-3 bg-gray-200 form-input w-full @error('email') border-red-500 border @enderror" name="email" value="{{ old('email') }}"  autocomplete="email" autofocus>
