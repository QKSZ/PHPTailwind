...
@section('content')
<h1 class="text-2xl text-center mt-10">Editar Proyecto {{$proyecto->titulo}}</h1>


    <form class="max-w-lg mx-auto my-10" action="{{route('proyectos.update',['proyecto'=> $proyecto->id])}}" method="POST">
    ...
    <input id="titulo" type="text" class="p-3 bg-gray-100 form-input w-full @error('password') is-invalid @enderror" name="titulo" placeholder="Titulo del Proyecto" value="{{$proyecto->titulo}}">
        @error('titulo')
    ...
    <option
      ...
      {{$proyecto->categoria_id== $categoria->id ? 'selected' : ''}}
      value="{{ $categoria->id }}">
      {{$categoria->nombre}}
    </option>
    @endforeach
    ...
    <option
      {{$proyecto->ubicacion_id== $ubicacion->id ? 'selected' : ''}}
      value="{{ $ubicacion->id }}">
      {{$ubicacion->nombre}}
    </option>
    @endforeach
    ...
    <div class="editable p-3 bg-gray-100 rounded form-input w-full text-gray-700 ">
      ...
      <input type="hidden" name="descripcion" id="descripcion" value="{{$proyecto->descripcion}}">
      @error('descripcion')
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mt-3 mb-6" role="alert">
    ...
    <input type="hidden" name="imagen" id="imagen" value="{{$proyecto->imagen}}">
  ...
  const dropzoneUlatina = new Dropzone('#dropzoneUlatina',{
    ...
    init: function(){
      if (document.querySelector('#imagen').value.trim()){
        ...
        imagenPublicada.nombreSevidor = document.querySelector('#imagen').value;
